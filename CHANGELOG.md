...
### Added
- **RFC 0005 Step 2 — OAuth 2.1 JWT verifier + scope gate** — lands the enforcement side of RFC 0005 on top of the Step-1 discovery layer ([#138](https://github.com/heznpc/AirMCP/pull/138)). New `src/server/oauth-verifier.ts` wraps `jose` with a lazy `createRemoteJWKSet`, pins `algorithms: [RS256, ES256]` (symmetric + `none` excluded — RFC 0005 R4 key-confusion hardening), and enforces `issuer`/`audience`/`exp`/`nbf` with a 60-second clock tolerance (RFC 0005 R2). Verification returns a typed `VerifyResult` union — `wrong_audience`, `wrong_issuer`, `expired`, `invalid_signature`, `jwks_unreachable`, `unsupported_alg`, `malformed_claims` — so the middleware can distinguish a bad-token 401 from an AS-down 503 and set `Retry-After` accordingly. JWKS unreachable never takes the server down at boot; first request pays the fetch, then jose's internal rotation keeps keys fresh. The middleware in `http-transport.ts` branches: `with-oauth*` policies run the JWT verify path, `with-token*` stays on the constant-time Bearer path, everything else skips auth entirely. On success, OAuth claims (subject + scopes + raw payload) enter the `AsyncLocalStorage` request context (`src/shared/request-context.ts`) and propagate through the MCP SDK dispatch down to the tool-registry pre-handler without any SDK changes. On failure, the 401 response carries an RFC 6750 `WWW-Authenticate: Bearer resource="…", error="invalid_token", error_description="<reason>"` header so conforming clients can retry with the right audience. Tool-registry pre-handler (RFC 0005 §3.4) runs `evaluateScopeGate` from `src/shared/oauth-scope.ts` before the rate-limit bucket: `mcp:read` for `readOnlyHint: true`, `mcp:write` for non-destructive writes, `mcp:destructive` for `destructiveHint: true`, `mcp:admin` pinned to `audit_log` / `audit_summary` / `memory_forget` / `setup_permissions` regardless of annotations so audit-trail access can't be approved under a bare read token. Scope hierarchy is cumulative (admin → destructive → write → read) so authorization servers don't have to mint multi-scope sets. Denials throw `[forbidden] scope <required> required for tool "<name>"` and audit with `status: error` before the handler runs, so a denied destructive call never burns the destructive rate bucket. Absent context (stdio, loopback-only, legacy Bearer) stays a full no-op so existing deployments are unchanged. 37 new tests across `tests/oauth-scope.test.js` (pure scope matrix + lattice), `tests/oauth-verifier.test.js` (end-to-end with a real RSA keypair + stubbed JWKS localhost server: happy path, wrong aud, wrong iss, expired, tampered signature, HS256 rejection, JWKS unreachable, `scp` array normalization, missing-sub rejection), and `tests/tool-registry-scope-gate.test.js` (scope matrix across read / write / destructive / admin tools + no-context no-op). `jose` 6.2.2 added as a direct dep; `npm audit --omit=dev` still 0 findings. New `npm run dev:oauth` launches a Keycloak 26 devcontainer with a version-controlled `airmcp` realm (`dev/dev` user + `mcp:*` scopes) via `docker/docker-compose.dev-oauth.yml` + `docker/keycloak-realm.json` — zero-interaction local OAuth flow for RFC 0005 R5.
