...
import { buildServerCard, buildOAuthProtectedResourceCard, isOAuthPolicy } from "./well-known-card.js";
...
export function validateNetworkPolicy(ctx: {
  ...
  switch (ctx.policy) {
    ...
    case "with-oauth+origin":
      // RFC 0005 Step 1 — discovery-only. Token validation middleware
      // is a follow-up; startup still refuses missing/invalid config
      // so the .well-known card never advertises OAuth pointing at
      // nothing.
      if (!ctx.oauthIssuer) {
        throw new Error(
      ...
      if (!/^https:\/\//.test(ctx.oauthIssuer)) {
        // Plain http issuers let a MITM mint or swap tokens — refuse
        // at startup rather than paper over with a runtime warning.
        throw new Error(`AIRMCP_OAUTH_ISSUER must be an https:// URL (got "${ctx.oauthIssuer}").`);
      }
      ...
      console.error(
        `[AirMCP] allowNetwork=${ctx.policy} — Step 1 (discovery-only). ` +
          "Do NOT bind to a public interface until token validation middleware lands.",
      );
      return;
...
export async function startHttpServer(options: HttpServerOptions): Promise<void> {
  ...
  app.get("/.well-known/oauth-protected-resource", (_req, res) => {
    if (!isOAuthPolicy(allowNetwork) || !oauth?.issuer || !oauth.audience) {
