...
> **2026-07-10 플랫폼 경계 정정:** macOS는 Shortcuts 안의 App Intent action을 지원하지만 `AppShortcutsProvider` 기반 App Shortcuts/추천 Siri phrase는 지원하지 않는다. 이 문서의 App Shortcuts·Siri phrase 설계는 iOS 전용이며, macOS는 생성된 App Intent action만 컴파일한다.

---

## Current Status (2026-07-10)

The iOS target is an engineering preview, not the macOS shipping surface and
not an App Store-ready product. Its Swift MCP server and in-process App Intent
router expose the same **exact eight-tool, read-only** contract:

1. `get_location_permission`
2. `list_calendars`
3. `list_contacts`
...
`IOSPreviewContract` rejects every other tool at registration and again at
direct App Intent dispatch. All eight tools declare `readOnly = true` and
`destructive = false`. There are no current iOS writes, HealthKit tools, Notes
tools, or Foundation Models/`Ask AirMCP` agent in this executable contract.

- [x] AirMCPKit shared Swift package and iOS SwiftUI shell
- [x] Token-protected Swift MCP HTTP server
- [x] Exact eight-tool registration and direct-dispatch allowlist
- [x] Generated App Intent types share the macOS tool metadata catalog
- [x] iOS `AppShortcutsProvider` entries match the exact executable preview
      allowlist; `Ask AirMCP` is not advertised as an App Shortcut
- [ ] Port macOS-equivalent HITL, HMAC audit, rate-limit, and emergency-stop
      governance before any iOS write tool is considered
- [ ] Reconcile every `ios/AppStore/` claim and screenshot with the executable
      contract; **App Store submission is HOLD until this gate passes**
- [ ] HealthKit, Interactive Snippets, Foundation Models host mode, and Liquid
      Glass remain roadmap work, not current release capabilities

> 2026-06-17 correction: Apple first-party WWDC26 material confirms a split,
> not an iOS "system MCP" bridge. App Intents/App Schemas connect apps to Siri,
> Shortcuts, Spotlight, and Apple Intelligence; MCP is public in Apple's
> developer-agent/Xcode layer. Treat iOS App Intents and iOS MCP HTTP as two
> separate AirMCP surfaces unless Apple ships a public consumer MCP API.
...
1. **Apple은 소비자/Siri 경로를 App Intents/App Schemas로, 개발자/agent 경로를 MCP/Xcode로 분리했다** — AirMCP iOS는 "시스템 MCP 자동 등록"이 아니라 App Intents 표면과 MCP HTTP 표면을 같은 도구 메타데이터에서 함께 내는 쪽이 맞다.
...
|               | macOS (현재)                                        | iOS preview / roadmap                                      |
| ------------- | --------------------------------------------------- | ---------------------------------------------------------- |
| 핵심 가치     | 모든 앱을 AI로 제어                                 | 현재: read-only PIM preview / 목표: on-device AI 허브       |
| 자동화 방식   | JXA (osascript)                                     | 네이티브 프레임워크 직접 호출                              |
| MCP 서버      | Node.js (TypeScript)                                | Swift (embedded in app)                                    |
| 트랜스포트    | stdio + HTTP (`with-token`/`with-oauth` — RFC 0005) | HTTP MCP + App Intents/Shortcuts/Siri                     |
| UI 표면       | MCP Apps iframes (Claude 등)                        | **MCP Apps iframes + iOS Interactive Snippets SwiftUI 뷰** |
| 자체 에이전트 | 외부 클라이언트(Claude/ChatGPT) 의존                | 현재 없음; Foundation Models 호스트 모드는 roadmap          |
| 도구 수       | 270+ (run-time 등록 ~111; smoke 기준)               | **현재 8개 read-only**; 40/92개는 과거 roadmap 목표    |
| 배포          | npm (`npx airmcp`)                                  | engineering preview; App Store **HOLD**                    |

**핵심 전략**: 공유 Swift Package(`AirMCPKit`)를 macOS/iOS 양쪽에서 사용. **Phase 2부터는 MCP 툴 메타데이터를 App Intent로 자동 생성** (RFC 0007)한다. macOS에서는 Shortcuts action, iOS에서는 추가로 Siri/App Shortcuts/Spotlight/App Schemas 표면을 같은 메타데이터 계약에서 파생한다. Phase 3의 Foundation Models 자체 에이전트 루프는 preview다.

> **문서 해석 규칙:** 아래의 40/51/75/92개 도구, HealthKit, write
> action, Foundation Models, `Ask AirMCP`, App Store 설계는 roadmap이거나
> 예시이다. 현재 바이너리의 능력 주장으로 재사용하면 안 된다.
...
/// App Intent — Siri / Shortcuts / Apple Intelligence
...
**Track B: App Intents / Siri / Shortcuts (MCP 아님)**
...
- Apple 1차 출처상 App Intents는 Siri/Shortcuts/Spotlight/Apple Intelligence 통합 경로다.
- MCP 클라이언트용 표면은 Track A HTTP 서버가 담당한다.
- App Intents는 "모든 MCP 클라이언트에서 자동 사용"이 아니라 Apple system surface용 별도 배포 채널이다.
- 따라서 App Intent codegen은 여전히 중요하지만, MCP transport 대체물로 설계하지 않는다.
...
### 7.1 iOS 로드맵 모듈 매핑 (현재 등록 카탈로그 아님)
...
**로드맵 수치:** Phase 1 ~51개 / 전체 ~92개 이상. 이는 지금
실행 가능한 도구 수가 아니다. 현재 iOS preview는 문서 상단에 고정된
8개 read-only tool만 등록합니다.
...
App Intents
    ...
    └── App Schemas / App Entities / semantic index

MCP HTTP
    │
    └── Claude / Codex / Cursor / Raycast / other MCP clients
```

Apple 1차 출처상 App Intents는 Siri/Apple Intelligence에 앱 기능과 콘텐츠를 노출하는 경로이고, MCP는 Xcode/agent tooling 쪽에서 공개적으로 확인된다. 따라서 iOS 전략은 **HTTP MCP 서버 + App Intents/App Schemas codegen**의 양면 전략이다. 모든 도구를 App Intent로도 생성하는 이유는 MCP 자동 변환이 아니라, Siri/Shortcuts/Spotlight와 같은 Apple system surface에서 같은 워크플로우를 재사용하기 위해서다.

### 8.2 iOS 전용 App Shortcuts (Siri 트리거)

> 아래는 후속 설계 예시이며 현재 preview 계약이 아닙니다. 현재
> `AppShortcutsProvider`에서 실행 가능하다고 제공할 수 있는 액션은 상단의
> 8개 read-only allowlist에 포함되어야 합니다. `HealthSummaryIntent`,
> `CreateReminderIntent`, `AskAirMCPIntent`는 현재 iOS 배포 계약이 아닙니다.
...
Track B: App Intents / Shortcuts / Siri
├── 앱이 실행 중이 아니어도 동작
├── 시스템이 intent 실행을 관리
└── Apple system surface에서는 백그라운드 제약 완화
...
### Phase 1: Foundation (roadmap, 8–10주)
...
### Phase 2: Intelligence + Search (roadmap, 4–6주)
...
### Phase 3: iOS-Only + App Store (roadmap, 4–6주)

- HealthModule (HealthKit) ← iOS 최대 차별화
- FocusModule (ManagedSettings)
- Cross-module 워크플로우
- App Store 제출
- App Intents/App Schemas codegen 대응 (**RFC 0007** 구현, §15.1)
- **Interactive Snippets 렌더러** (outputSchema → SwiftUI 뷰, §15.5)
- **FoundationModels 호스트 모드** (오프라인 에이전트 루프, §15.3)

**Deliverable**: App Store 출시, ~92개 도구, **HTTP MCP 서버 + App Intents/App Schemas + 오프라인 에이전트**
...
| **App Intent 전략** (2026-06)   | **자동 codegen**: MCP 툴 메타데이터 → `AppIntent` / `AppEntity` / App Schemas 어댑터                           | 툴마다 수동 App Intent 복제                    | 270+ 툴을 수동 포팅 불가; 메타데이터 하나로 MCP와 Apple system surface 유지 |
...
### 15.1 WWDC26 정정: App Intents와 MCP는 공개 표면이 다르다

[Apple WWDC26 Apple Intelligence guide](https://developer.apple.com/wwdc26/guides/apple-intelligence/) — App Intents/App Schemas는 앱을 Siri AI, Shortcuts, Spotlight, Apple Intelligence에 연결하는 경로다. 같은 WWDC26의 [Platforms State of the Union](https://developer.apple.com/videos/play/wwdc2026/102/)은 MCP를 Xcode/coding-agent 도구 계층에서 확인한다. 즉 공개된 1차 출처 기준으로는 **consumer/Siri path = App Intents**, **developer/agent path = MCP/Xcode**이며, iOS 앱의 App Intent가 모든 MCP 클라이언트에 자동 노출된다는 공개 계약은 없다.

**AirMCP 응수** → **RFC 0007 + P1b AppEntity/AppSchema scaffold**: MCP 툴 메타데이터를 `AppIntent`/`AppEntity`/App Schemas로 자동 생성해 Siri/Shortcuts/Spotlight/Apple Intelligence 표면을 얻는다. MCP 클라이언트는 계속 HTTP/stdio transport를 통해 붙고, iOS 앱은 로컬 HTTP MCP 서버를 별도 유지한다. 기존 "iOS 26.1 시스템 MCP 자동 등록" 가설은 2026-06-17 기준 반려한다.
...
**App Intents / Shortcuts (Track B — Apple system surface, not MCP config)**:

No MCP client JSON is expected here. Users invoke App Intents through Siri,
Shortcuts, Spotlight, widgets, controls, or Apple Intelligence surfaces. MCP
clients should use Track A unless Apple publishes a consumer MCP transport.
